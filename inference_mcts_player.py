...
The policy head supplies P(s, a); the value head supplies V(s) at expanded
leaves; leaves are batched into one forward pass per descent group, with
virtual loss steering parallel descents to distinct paths. A softening
temperature (default 1.5) is applied to the policy logits before softmax,
because the one-hot supervised policy is too peaky to be a usable PUCT prior.
"""
