...
PALETTES: dict[str, dict] = {
    'gold': {
        ...
        'faint':        '#4A4438',
        'cmap_stops':   [
            (0.00, '#2A2218'), (0.30, '#5C4A20'), (0.55, '#C9A43C'),
            (0.80, '#E2C060'), (1.00, '#F5E5A8'),
        ],
    ...
    'redblue': {
        ...
        'faint':        '#3A4258',
        'cmap_stops':   [
            (0.00, '#0E1830'), (0.30, '#2C4882'), (0.55, '#7B4E96'),
            (0.80, '#E0556E'), (1.00, '#FF8E78'),
        ],
    ...
    'terminal': {
        ...
        'faint':        '#4A4A4A',
        'cmap_stops':   [
            (0.00, '#0A1F0A'), (0.30, '#1A5A1A'), (0.55, '#36D936'),
            (0.80, '#74FF74'), (1.00, '#CCFFCC'),
        ],
...
def _resolve_palette(name: str) -> dict:
    p = PALETTES[name]
    return {**p, 'cmap': LinearSegmentedColormap.from_list(
        f'chess_{name}', p['cmap_stops'])}
...
EDGE_COLOR = SPARK_COLOR = (0, 0, 0)
BOARD_BORDER = DEMO_COLOR = ''
TITLE_FAMILY = TITLE_STYLE = TITLE_WEIGHT = ''
TITLE_CAPS = False
...
def load_model_or_random() -> tuple[ChessModel, bool]:
    try:
        return load_trained_model(), False
    except (FileNotFoundError, RuntimeError, ValueError) as exc:
        warnings.warn(f"Falling back to random-init model: {exc}")
        return ChessModel().eval(), True
...
def capture_activations(model: ChessModel, board_t: torch.Tensor,
    ...
    for module in model.cnn.children():
        if isinstance(module, ResidualBlock):
            hooks.append(module.register_forward_hook(make_hook(f'res_{block_idx}')))
            block_idx += 1
    hooks.append(model.lstm.register_forward_hook(make_hook('lstm')))
    fc_relu_idx = 0
    for sub in model.fc.children():
        if isinstance(sub, nn.ReLU):
            hooks.append(sub.register_forward_hook(make_hook(f'fc_{fc_relu_idx}')))
            fc_relu_idx += 1
...
def build_layer_specs(captures: dict[str, torch.Tensor],
    ...
    input_acts = np.abs(board_tensor).mean(axis=(0, 1))
    layers.append(LayerSpec('input', 'input', _normalize(input_acts)))

    init_conv = captures['initial_conv'].numpy()[0]
    ...
    for i, key in enumerate(sorted(k for k in captures if k.startswith('res_'))):
        ...
        layers.append(LayerSpec(
            key, f'res{i+1}', _normalize(block_means[init_idx]),
        ))

    lstm_h = captures['lstm'].numpy()[0]
    lstm_final = lstm_h[-1] if lstm_h.ndim == 2 else lstm_h
    _, lstm_sel = _topk_abs(lstm_final, MAX_NODES_PER_COLUMN)
    layers.append(LayerSpec('lstm_hidden', 'lstm', _normalize(lstm_sel)))
    ...
    for i, key in enumerate(sorted(k for k in captures if k.startswith('fc_'))):
        ...
        layers.append(LayerSpec(key, f'fc{i+1}', _normalize(sel)))
    ...
    policy_spec = LayerSpec('policy', 'policy', _normalize(policy_probs[p_idx]))
    ...
    value_spec = LayerSpec(
        'value', 'value',
        np.array([abs(value_scalar)], dtype=np.float32),
    )
...
def position_layers(layers: list[LayerSpec], rng: np.random.Generator,
    ...
    for i, layer in enumerate(layers):
        ...
        if m == 1:
            ...
            layer.y_positions = ys + rng.uniform(-0.012, 0.012, size=m)
        layer.node_sizes = 14 + 64 * layer.activations
...
def build_synapses(layers: list[LayerSpec],
    ...
    for src, dst in zip(layers[:-1], layers[1:]):
        ...
        if n_src * n_dst <= MAX_SYNAPSES_PER_GAP:
...
def pick_sparks(synapses: list[SynapseSet]) -> list[dict]:
    sparks = []
    for gap_idx, syn in enumerate(synapses):
        chosen = np.argsort(-syn.activations)[:SPARKS_PER_GAP]
        sparks.append({
...
PIECE_FILENAME = {
    (chess.PAWN,   chess.WHITE): 'P_w.png', (chess.PAWN,   chess.BLACK): 'P_b.png',
    (chess.KNIGHT, chess.WHITE): 'N_w.png', (chess.KNIGHT, chess.BLACK): 'N_b.png',
    (chess.BISHOP, chess.WHITE): 'B_w.png', (chess.BISHOP, chess.BLACK): 'B_b.png',
    (chess.ROOK,   chess.WHITE): 'R_w.png', (chess.ROOK,   chess.BLACK): 'R_b.png',
    (chess.QUEEN,  chess.WHITE): 'Q_w.png', (chess.QUEEN,  chess.BLACK): 'Q_b.png',
    (chess.KING,   chess.WHITE): 'K_w.png', (chess.KING,   chess.BLACK): 'K_b.png',
}
...
def draw_static_chrome(ax, board: chess.Board, used_random_init: bool,
                       layout: LayoutConfig, width: int, height: int):
    ...
    if layout.show_board:
        ...
        ax.imshow(board_img, extent=(x0_b, x1_b, y_top - y_height, y_top),
...
def draw_readout(ax, policy_spec: LayerSpec, value_scalar: float,
                 board: chess.Board, flip: bool) -> dict:
    try:
    ...
    def _label(x, anchor):
        return ax.text(x, 0.135, '', ha=anchor, va='center', color=TITLE_COLOR,
                       fontsize=15, family=TITLE_FAMILY,
                       fontstyle=TITLE_STYLE, fontweight=TITLE_WEIGHT,
                       zorder=6, alpha=0.0)

    def _caption(x, anchor, text):
        return ax.text(x, 0.085, text, ha=anchor, va='center', color=FAINT_COLOR,
                       fontsize=7.5, family=body_family, zorder=6, alpha=0.0)

    return {
        'move_label':    _label(0.21, 'left'),
        'move_caption':  _caption(0.21, 'left',  move_caption_text),
        'value_label':   _label(0.94, 'right'),
        'value_caption': _caption(0.94, 'right', value_caption_text),
        'best_move_text': best_label,
        'value_scalar':   float(value_scalar),
    }


def init_node_artists(ax, layers: list[LayerSpec]) -> list[dict]:
    artists = []
    for layer in layers:
        base = layer.node_sizes
        color = NODE_CMAP(0.6) if layer.name != 'value' else (1, 1, 1, 1)
        xs = [layer.x] * layer.activations.size
        halo = ax.scatter(xs, layer.y_positions, s=base * 12.0, c=[color],
                          alpha=0.0, edgecolors='none', zorder=3)
        mid  = ax.scatter(xs, layer.y_positions, s=base * 4.0, c=[color],
                          alpha=0.0, edgecolors='none', zorder=4)
        core = ax.scatter(xs, layer.y_positions, s=base, c=[color],
                          alpha=0.95, edgecolors='none', zorder=5)
        artists.append({'halo': halo, 'mid': mid, 'core': core, 'layer': layer})
    return artists


def init_synapse_artists(ax, synapses: list[SynapseSet]) -> list[LineCollection]:
    lcs = []
    for syn in synapses:
        segs = np.stack([
        ...
        lc = LineCollection(
            ...
            linewidths=0.4 + 1.6 * syn.activations, zorder=2,
        )
...
def init_spark_artists(ax, sparks: list[dict]):
    return [
        ax.scatter(s['x_src'], s['y_src'], s=24, c=[SPARK_COLOR],
                   alpha=0.0, edgecolors='none', zorder=6)
        for s in sparks
    ]


def update_frame(frame: int, total_frames: int, node_artists,
    ...
    for entry in node_artists:
        ...
        for k in ('core', 'mid', 'halo'):
            entry[k].set_facecolor(colors)
    ...
    for syn, lc in zip(synapses, synapse_lcs):
        env = _gaussian(0.5 * (syn.x_src + syn.x_dst), p, PULSE_SIGMA * 1.4)
        alphas = 0.05 + 0.60 * env * (0.3 + 0.7 * syn.activations)
        rgba = np.concatenate(
            [np.tile(EDGE_COLOR, (alphas.size, 1)), alphas[:, None]], axis=1,
        )
        lc.set_color(rgba)

    for s, art in zip(sparks, spark_artists):
        gap_xs, gap_xe = s['x_src'][0], s['x_dst'][0]
        local_t = (0.5 if gap_xe == gap_xs
                   else float(np.clip((p - gap_xs) / (gap_xe - gap_xs), 0.0, 1.0)))
        env = _gaussian(s['x_mid'].mean(), p, PULSE_SIGMA * 1.2)
        art.set_offsets(np.column_stack([
            s['x_src'] + local_t * (s['x_dst'] - s['x_src']),
            s['y_src'] + local_t * (s['y_dst'] - s['y_src']),
        ]))
    ...
    if readout is not None:
        ...
        settled = readout['value_scalar'] * value_reveal
...
def render_all_frames(fig, total_frames, update_fn) -> list[np.ndarray]:
    frames = []
...
def save_gif(frames: list[np.ndarray], path: Path, source_fps: int,
    ...
    # Single shared palette across frames keeps the GIF compact.
    palette_src = Image.fromarray(sampled[int(0.83 * len(sampled))]).quantize(
    ...
    pil_frames[0].save(
        ...
        duration=int(1000 / effective_fps), loop=0,
...
def _apply_palette(name: str) -> None:
    ...
    p = _resolve_palette(name)
    BG_COLOR, TITLE_COLOR, SUBTLE_COLOR, FAINT_COLOR = (
        p['bg'], p['title'], p['subtle'], p['faint'])
    NODE_CMAP, EDGE_COLOR, SPARK_COLOR = (
        p['cmap'], p['edge_rgb'], p['spark_rgb'])
    BOARD_BORDER, DEMO_COLOR = p['board_border'], p['demo_color']
    TITLE_FAMILY, TITLE_STYLE, TITLE_WEIGHT, TITLE_CAPS = (
        p['title_family'], p['title_style'],
        p['title_weight'], p['title_caps'])
...
def main():
    ...
    for uci in (m.strip() for m in args.moves.split(',') if m.strip()):
        board.push(chess.Move.from_uci(uci))

    print(f'[info] palette={args.palette}  layout={layout.name}')
    print(f'[info] position={board.fen()}')
    print(f'[info] arch={RESIDUAL_BLOCKS}x ResNet ({RESIDUAL_FILTERS} filters)')
    ...
    position_layers(
        layers, rng,
        x_left=layout.layer_x_left,  x_right=layout.layer_x_right,
        y_low=layout.layer_y_low,    y_high=layout.layer_y_high,
    )

    synapses = build_synapses(layers, rng)
    sparks   = pick_sparks(synapses)

    fig, ax = setup_figure(args.width, args.height)
    draw_static_chrome(ax, board, used_random_init, layout,
    ...
    def update(frame):
        update_frame(frame, args.frames, node_artists,
    ...
    frames = render_all_frames(fig, args.frames, update)

    sfx = args.suffix
    output_stem = f'{args.output_name}{sfx}'
    poster_stem = f'{args.poster_name or args.output_name + "_poster"}{sfx}'
    save_poster(frames[int(0.83 * args.frames)],
                out_dir / f'{poster_stem}.png')
    _report(out_dir / f'{poster_stem}.png')

    if not args.no_mp4:
        save_mp4(frames, out_dir / f'{output_stem}.mp4', fps=args.fps)
        _report(out_dir / f'{output_stem}.mp4')
    if not args.no_webm:
        try:
            save_webm(frames, out_dir / f'{output_stem}.webm', fps=args.fps)
            _report(out_dir / f'{output_stem}.webm')
        except Exception as exc:
            print(f'[warn] WebM encoding failed: {exc}')
    if not args.no_gif:
        save_gif(frames, out_dir / f'{output_stem}.gif', source_fps=args.fps)
        _report(out_dir / f'{output_stem}.gif')
